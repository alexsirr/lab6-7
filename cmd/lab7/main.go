...
func main() {
	...
	router.GET("/query1", func(c *gin.Context) {
		...
		// put your query here
		rows, err := db.Query("SELECT COUNT(Person) FROM Person WHERE id IN (SELECt userid FROM Machine JOIN OS ON osid = OS.id WHERE OS.type = 'Windows 10');") // <--- EDIT THIS LINE
		if err != nil {
		...
		var count int      // <--- EDIT THESE LINES

		for rows.Next() {
			// assign each of them, in order, to the parameters of rows.Scan.
			// preface each variable with &
			rows.Scan(&count) // <--- EDIT THIS LINE
			// can't combine ints and strings in Go. Use strconv.Itoa(int) instead
			table += "<tr><td>" + strconv.Itoa(count) + "</td></tr>"
		}
	...
	router.GET("/query2", func(c *gin.Context) {
		table := "<table class='table'><thead><tr>"
		// put your query here
		rows, err := db.Query("SELECT fname, lname FROM Person JOIN Machine ON Person.id = userid WHERE lastsignin > '2016-01-01 00:00:00';") // <--- EDIT THIS LINE
		if err != nil {
			// careful about returning errors to the user!
			c.AbortWithError(http.StatusInternalServerError, err)
		}
	...
	router.GET("/query3", func(c *gin.Context) {
		table := "<table class='table'><thead><tr>"
		// put your query here
		rows, err := db.Query("SELECT fname, lname FROM Person WHERE id IN (SELECT userid FROM Machine JOIN OS ON OS.id = osid WHERE type LIKE 'OSX%');") // <--- EDIT THIS LINE
		if err != nil {
			// careful about returning errors to the user!
			c.AbortWithError(http.StatusInternalServerError, err)
		}
